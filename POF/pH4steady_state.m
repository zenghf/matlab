...
%  steady state, at every time point, set I & S spin polarization to
%   initial spin state of parahydrogen
%% define spin system
nSpin = 4;
csI = 0; csS = 0; csR = 10; csT = 10;
JIS = 7; JIR = 3; JIT = 0.1;
JSR = 0.1; JST = 10; JRT = 7;
ts = linspace(0,506,50);
B = 100e-4;
coupling = 'strong';
...
for k = 1:length(ts)
    ...
    for n = 1:length(comp)
        ...
        if isempty([strfind(comp(n).opNotation, 'I1'),strfind(comp(n).opNotation, 'I2')])
            op = op + comp(n).coeff * comp(n).opMat;
        end
    end
    if k < length(ts)
        op = op + op0;
        dt = ts(k+1)-ts(k);
        op = evolve(op, spinSys, dt, B, coupling);
    end
end
validTerm = any(res,1);
